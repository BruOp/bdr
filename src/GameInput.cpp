...
namespace
{
    ...
    void KbmInitialize()
    {
        ...
        if (FAILED(s_Keyboard->SetCooperativeLevel(GameInput::g_hWnd, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE)))
        ...
        if (FAILED(s_Mouse->SetCooperativeLevel(GameInput::g_hWnd, DISCL_FOREGROUND | DISCL_EXCLUSIVE)))
    ...
    void KbmUpdate()
    {
        ...
        if (foreground != GameInput::g_hWnd // wouldn't be able to acquire
            || !visible) {
            KbmZeroInputs();
        }
        else {
